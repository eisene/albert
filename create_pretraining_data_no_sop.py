...
flags.DEFINE_bool(
    "keep_accents", False,
    "Whether to keep accents in the input text. False by default. Make "
    "sure to set to True if accents in your language contain important "
  ...
  while i < len(document):
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:
      if current_chunk:
        ...
        # (first) sentence.
        chunk_tokens = []
        for cur_chunk_tokens in current_chunk:
          chunk_tokens.extend(cur_chunk_tokens)
        truncate_seq(chunk_tokens, max_num_tokens, rng)

        assert len(chunk_tokens) >= 1
